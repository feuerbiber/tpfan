...
class Dashboard(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._max_labels: dict[str, QLabel] = {}
        self._max: dict[str, float] = {}
        grid = QGridLayout(self)
        grid.addWidget(QLabel(""),         0, 0)
        grid.addWidget(QLabel("aktuell"),  0, 1)
        grid.addWidget(QLabel("Max"),      0, 2)
        for i, name in enumerate(self.SENSOR_ORDER):
            row = i + 1
            grid.addWidget(QLabel(name + ":"), row, 0)
            v = QLabel("--")
            m = QLabel("--")
            grid.addWidget(v, row, 1)
            grid.addWidget(m, row, 2)
            self._labels[name] = v
            self._max_labels[name] = m
        self.cpu_label = self._labels["CPU"]
        self.gpu_label = self._labels["GPU"]
        self.fan1_label = QLabel("--")
        self.fan2_label = QLabel("--")
        self.level_label = QLabel("--")
        row = len(self.SENSOR_ORDER) + 1
    ...
    def apply_tick(self, p: TickPayload) -> None:
        for name, lbl in self._labels.items():
            v = p.temps.get(name)
            mlbl = self._max_labels[name]
            if v is None:
                lbl.setText("--")
                mlbl.setText("--" if name not in self._max else f"{self._max[name]:.1f} °C")
                continue
            self._max[name] = max(self._max.get(name, v), v)
            lbl.setText(f"{v:.1f} °C")
            mlbl.setText(f"{self._max[name]:.1f} °C")
